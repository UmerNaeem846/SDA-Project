...
def show_dashboard(config, data, result):
    ...
    #Generate charts
    region_country_wise_plot(config,data)
    year_wise_plot(data)


def region_country_wise_plot(config,data):
    ...
    #Sum GDP per region
        ...
        for row in data:
            country_totals[row["Country"]] += row["Value"]

        country = list(country_totals.keys())
        values = list(country_totals.values())

        #Bar Chart
        plt.figure()
        plt.bar(country, values, color='skyblue')
        plt.title("Country-wise GDP (Bar Chart)")
        plt.xlabel("Country")
        plt.ylabel("GDP Value")
        plt.show()
        ...
        #Pie Chart
        plt.figure()
        plt.pie(values, labels=country, autopct="%1.1f%%", startangle=140)
        plt.title("Country-wise GDP Distribution (Pie Chart)")
        plt.show()
